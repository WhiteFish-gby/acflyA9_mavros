...
bool Altitude_Control_Enable(double TIMEOUT)
{
	...
	if (LockCtrl(TIMEOUT))
	{
		...
		bool tmp_suc;
		tmp_suc = Attitude_Control_Enable();
...
void ctrl_Position()
{
	...
	get_throttle_force(&AccelerationENU.z);
	AccelerationENU.z -= GravityAcc;
	...
	{ //水平位置控制
		...
		case Position_ControlMode_Position:
		{
			if (inFlight)
			{
				...
				TargetVelocity_1.x = T2_1.x;
				TargetVelocity_1.y = T2_1.y;
		...
		case Position_ControlMode_RouteLine:
		{
			if (inFlight)
			{
				...
				TargetVelocity_1.x = T2r_1.x + T2d_1.x;
				TargetVelocity_1.y = T2r_1.y + T2d_1.y;
